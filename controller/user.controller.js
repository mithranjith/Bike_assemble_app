...
const { to, isNull, ReE, ReS, isEmpty } = require("../services/util.services");
...
const login = async function (req, res) {
  ...
  if (isNull(body.email))
  ...
  else if (isNull(body.bike))
    return ReE(
      res,
      {
        message: "Please select a bike to login",
        success: false,
      },
      HttpStatus.BAD_REQUEST
    );
    ...
    [err, bike] = await to(
      BikesModel.findOne({ _id: new ObjectId(body.bike), active: true })
    );

    if (err) {
      console.log("Error while fetching bike...", err);
      return ReE(res, err, HttpStatus.INTERNAL_SERVER_ERROR);
    } else if (!bike) {
      return ReE(
        res,
        { message: "Bike not found or inactive" },
        HttpStatus.BAD_REQUEST
      );
    } else {
      let query = {
...
const updateUser = async function (req, res) {
  ...
  if (err) {
    ...
    [err, user] = await to(UsersModel.findOne(query));
    return ReS(res, { user: user }, HttpStatus.OK);
  }
};
